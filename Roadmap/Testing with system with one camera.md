...
Adding the alarm part in `main_system.py`
```
import cv2
import rospy
from sensor_msgs.msg import Image
...
def main():
    ...
    # NEW: Flag to ensure alarm only triggers once per Mode 4 entry
    ...
    while not rospy.is_shutdown():
        ...
        if not person_detected:
            ...
            if video_writer is not None:
                video_writer.release()
                video_writer = None
            # Reset the alarm flag when threat is gone, ready for next trigger
            alarm_triggered = False
        else:
            distance = latest_depth_m
            if distance > 4.0:
                ...
                current_mode = 4
                # Only trigger the alarm ONCE when first entering Mode 4
                if not alarm_triggered:
                    os.system("mpg123 -f 4500 /home/SaPHaRI/chester/resources/alarm.mp3 &")
                    print("[MODE 4] Alarm triggered!")
